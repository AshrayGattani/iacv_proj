...
def AccuracyTest():
	filename1 = filedialog.askopenfilename(initialdir = "../Forged%20Images/",title ="Open File",filetypes = (("png files","*.png"),("bmp files","*.bmp"),("jpeg files","*.jpg"),("All Files","*.*")))
	img_for_accuracy = cv2.imread(filename1,0)
	dp=0
	yp=0
	yn=0
...
def MakeDCT():
	...
	for i in range (0,height-7):
		for j in range(0,width-7):
			...
			count+=1
	messagebox.showinfo("Success","DCT success")
...
def TryToDetectForgery():
	...
	filename = filedialog.asksaveasfilename(initialdir = "../Forged%20Images/",title = "Save File",filetypes = (("png files","*.png"),("bmp files","*.bmp"),("jpeg files","*.jpg"),("All Files","*.*")))
	cv2.imwrite('filename.png',img2)
	result_image=PhotoImage(file='filename.png')
